...
namespace ASMC.Devices.USB_Device.SiliconLabs
{
    ...
    public class UsbExpressWrapper
    {
        ...
        public enum StatusCode
        {
            ...
            [Description("Не верный дескриптор.")]
            InvalidHandle,
        ...
        /// </summary>
        /// <param name="dwDeviceNum">Номер устройства (от 0 до значения, полученного вызовом <see cref="GetCountDevices"/> </param>
        ...
        /// </summary>
        /// <param name="dwDevice">Номер устройства (от 0 до значения, полученного вызовом <see cref="GetCountDevices"/></param>
        ...
        {
            if (dwBytesToRead > MaxReadSize) throw new ArgumentOutOfRangeException($"Считывание возможно не более {MaxReadSize} байт.");
        ...
        {
            if (dwBytesToWrite > MaxWriteSize) throw new ArgumentOutOfRangeException($"Запись возможна не более {MaxWriteSize} байт.");
            var code = NativeMethods.SI_Write(Handle, lpBuffer, dwBytesToWrite, ref lpdwBytesWritten, o);
            GenericException(nameof(GetProductString), (StatusCode)code);
        }

        public static StatusCode DeviceIoControl(IntPtr cyHandle, uint dwIoControlCode, byte[] lpInBuffer, uint dwBytesToRead, byte[] lpOutBuffer, uint dwBytesToWrite, ref uint lpdwBytesSucceeded)
        {
            return (StatusCode)NativeMethods.SI_DeviceIOControl(cyHandle, dwIoControlCode, lpInBuffer, dwBytesToRead, lpOutBuffer, dwBytesToWrite, ref lpdwBytesSucceeded);
        }
        ...
        public void FlushBuffers(bool clearTansmit, bool clearReceive)
        {
            ...
            var code = NativeMethods.SI_FlushBuffers(Handle, Convert.ToByte(clearTansmit), Convert.ToByte(clearReceive));
            GenericException(nameof(GetProductString), (StatusCode)code);
        }
        ...
        public static StatusCode SetFlowControl(IntPtr cyHandle, byte bCtsMaskCode, byte bRtsMaskCode, byte bDtrMaskCode, byte bDsrMaskCode, byte bDcdMaskCode, bool bFlowXonXoff)
        {
            return (StatusCode)NativeMethods.SI_SetFlowControl(cyHandle, bCtsMaskCode, bRtsMaskCode, bDtrMaskCode, bDsrMaskCode, bDcdMaskCode, bFlowXonXoff);
        }
        ...
        public static Version GetDllVersion()
        {
            ...
            GenericException(nameof(GetProductString), (StatusCode)code);
            return Version.Parse($"{(highVersion>>16)&0xffff}.{highVersion & 0xffff}.{(lowVersion >> 16) & 0xffff}.{lowVersion & 0xffff}");
        }
        /// <summary>
